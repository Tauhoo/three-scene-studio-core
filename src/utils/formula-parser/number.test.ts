...
describe('parseExpression', () => {
  ...
  test('start with 0', () => {
    ...
    const expected = { type: 'NUMBER', value: 0.123, text: '0.123' }
    expect(result).toEqual(expected)
  })

  test('start with 0 and long integer part', () => {
    const result = parse('023.123')
    const expected = { type: 'NUMBER', value: 23.123, text: '023.123' }
    expect(result).toEqual(expected)
  })

  test('end with 0', () => {
    const result = parse('123.00')
    const expected = { type: 'NUMBER', value: 123, text: '123.00' }
    expect(result).toEqual(expected)
  })

  test('start with 0 and long decimal part and end with 0', () => {
    const result = parse('23.4500')
    const expected = { type: 'NUMBER', value: 23.45, text: '23.4500' }
    expect(result).toEqual(expected)
  })
})
